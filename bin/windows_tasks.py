...
def parser():
    ...
    list_parser.add_argument('--path', '-p', default='\\', help='The location of the tasks to list')
    ...
    list_parser.add_argument('--summarize', '-s', action='store_true', help='Summarize')
...
def main():
    ...
    action = args.action
    if action == 'list':
        show_tasks(args.path, args.recursive, args.format, args.summarize)
    else:
        raise ValueError(f'Unexpected {action=!r}')


def show_tasks(path: Optional[str] = '\\', recursive: bool = False, out_fmt: str = 'pseudo-json', summarize=False):
