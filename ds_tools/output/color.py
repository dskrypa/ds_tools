"""
ANSI Color Handling (based on the 'colored' module)
...
def colored(text, color=None, bg_color=None, attrs=None, reset=True, *, prefix=None):
    parts = (
        (ESC + str(prefix) + END) if prefix else '',
...
def ansi_color_code(color, base):
    ...
    if color.isdigit():
        try:
            color = ANSI_COLORS_REVERSE[color]
        except KeyError as e:
            raise InvalidAnsiCode(color) from e
        return code + ANSI_COLORS[color] + END
    elif color.startswith('#'):
        try:
            return code + HEX_COLORS_REVERSE[color.lower()] + END
        except KeyError as e:
            raise InvalidAnsiCode(color) from e
    else:
        try:
